...
#passw=input("Enter your sql shell's password :")
passw=""

# Describing fonts to be used
fnt1 = ("Italic", 45, "bold")
fnt2 = ("Italic", 25, "bold")
fnt3 = ("Italic", 20, "bold")
fnt4 = ("Italic", 18, "bold")
...
def passwor():
    ...
    rootx = tk.Tk()
    rootx.minsize(width = 600, height = 300)
    rootx.maxsize(width = 600, height = 300)
    rootx.title('Enter your information')
    #frame for window margin
    passx = tk.Frame(rootx, padx=0, pady=0)
    passx.pack(fill=tk.BOTH, expand=True)
    img = ImageTk.PhotoImage(file="passwords1.png")
    label3 = Label(passx,image=img)
    label3.place(x=0, y=0)
    lbl1 = Label(passx, text='LOGIN', font=fnt1, bg='black',fg='yellow')
    lbl1.place(x=195,y=20)
    #entrys with not shown text
    lbl_newp = Label(passx, text='User Name:', font=fnt4, bg='black',fg='yellow')
    user = Entry(passx, font=fnt4, bg='black',fg='yellow', show='*')
    lbl_newp.place(x=70,y=120)
    user.place(x=230,y=120)
    lbl_news = Label(passx, text='Password :', font=fnt4,  bg='black',fg='yellow')
    password = Entry(passx, font=fnt4,  bg='black',fg='yellow', show='*')
    lbl_news.place(x=70,y=160)
    password.place(x=230,y=160)
    
    #button to attempt to login
    b = tk.Button(passx, borderwidth=4, text="ENTER",font= fnt4, width=10, command=check_password)
    b.place(x=210,y=235)
